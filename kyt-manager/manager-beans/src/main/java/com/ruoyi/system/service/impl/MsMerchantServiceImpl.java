package com.ruoyi.system.service.impl;

import java.util.List;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import com.ruoyi.system.mapper.MsMerchantMapper;
import com.ruoyi.system.domain.MsMerchant;
import com.ruoyi.system.service.IMsMerchantService;
import com.ruoyi.common.core.text.Convert;

/**
 * 【请填写功能名称】Service业务层处理
 * 
 * @author ruoyi
 * @date 2019-10-22
 */
@Service
public class MsMerchantServiceImpl implements IMsMerchantService 
{
    @Autowired
    private MsMerchantMapper msMerchantMapper;

    /**
     * 查询【请填写功能名称】
     * 
     * @param id 【请填写功能名称】ID
     * @return 【请填写功能名称】
     */
    @Override
    public MsMerchant selectMsMerchantById(Long id)
    {
        return msMerchantMapper.selectMsMerchantById(id);
    }

    /**
     * 查询【请填写功能名称】列表
     * 
     * @param msMerchant 【请填写功能名称】
     * @return 【请填写功能名称】
     */
    @Override
    public List<MsMerchant> selectMsMerchantList(MsMerchant msMerchant)
    {
        return msMerchantMapper.selectMsMerchantList(msMerchant);
    }

    /**
     * 新增【请填写功能名称】
     * 
     * @param msMerchant 【请填写功能名称】
     * @return 结果
     */
    @Override
    public int insertMsMerchant(MsMerchant msMerchant)
    {
        return msMerchantMapper.insertMsMerchant(msMerchant);
    }

    /**
     * 修改【请填写功能名称】
     * 
     * @param msMerchant 【请填写功能名称】
     * @return 结果
     */
    @Override
    public int updateMsMerchant(MsMerchant msMerchant)
    {
        return msMerchantMapper.updateMsMerchant(msMerchant);
    }

    /**
     * 删除【请填写功能名称】对象
     * 
     * @param ids 需要删除的数据ID
     * @return 结果
     */
    @Override
    public int deleteMsMerchantByIds(String ids)
    {
        return msMerchantMapper.deleteMsMerchantByIds(Convert.toStrArray(ids));
    }

    /**
     * 删除【请填写功能名称】信息
     * 
     * @param id 【请填写功能名称】ID
     * @return 结果
     */
    @Override
    public int deleteMsMerchantById(Long id)
    {
        return msMerchantMapper.deleteMsMerchantById(id);
    }
}
